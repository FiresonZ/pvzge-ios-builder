...
<details>
<summary><b>Q5：点击「开始游戏」后白屏一闪又回到主屏幕（🟡 待测试 / 待修复）</b></summary>

- **状态**：🟡 **待测试 / 待修复**（尚未定位根因）
- **现象**：包使用内嵌本地 http 服务器加载，主界面能正常进入；但点击「**开始游戏**」后，画面短暂白屏一下又跳回主屏幕，无法真正开始对局。
- **初步假设**：为旧机型（内存小 / GPU 较旧）在进入对局时一次性加载大量资源（地图、纹理、音频、wasm）导致内存峰值过高，被系统回收回到主界面。**此假设尚未验证**。
- **待测试项**：
  1. 在高配机型（A12+ / 更高内存）上是否同样复现；
  2. 用 **Lite 版**（资源已压缩）是否能缓解；
  3. 是否是进入对局那一次的资源懒加载 / 大并发请求导致。
- **暂不深究**：先记录在案，后续有真机复现样例、日志或更多机型反馈后再修。
</details>
